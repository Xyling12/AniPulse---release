...
fun ShikiImage?.posterUrl(): String? =
    this?.original?.takeUnless { it.isMissingImage() }?.let { com.animelib.app.data.Api.SHIKIMORI_IMAGES + it }

fun ShikiImage?.previewUrl(): String? =
    this?.preview?.takeUnless { it.isMissingImage() }?.let { com.animelib.app.data.Api.SHIKIMORI_IMAGES + it }
...
fun posterOf(id: Long, image: ShikiImage?): String =
    image.posterUrl() ?: (com.animelib.app.data.Api.GATEWAY + "poster/$id")
